...
function obj:togglePasteOnSelect()
	...
	hs.notify.show(
		"TextClipboardHistory",
		"Paste-on-select is now " .. (self.paste_on_select and "enabled" or "disabled"),
		""
	)
end
...
function obj:showClipboard()
	if self.selectorobj ~= nil then
		...
		hs.notify.show(
			"TextClipboardHistory not properly initialized",
			"Did you call TextClipboardHistory:start()?",
			""
		)
	end
end
